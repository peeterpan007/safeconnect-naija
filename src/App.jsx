...
import { IncidentProvider } from "./components/IncidentContext"; // ✅ add incidents provider
...
function AppContent() {
  ...
  const renderTabContent = () => {
    // 🚨 Guest restrictions applied here
    if (guest) {
      switch (activeTab) {
        case "home":
          return renderHomePage();
        case "incidents":
          return <IncidentReports guest={true} />; // ✅ guest can report + view
        case "map":
          return <IncidentMap />; // ✅ guest can view all incidents on map
        case "ads":
        case "community":
        case "events":
        case "news":
    ...
    <div>
      ...
      {(user || guest) && (
        <nav className="bottom-nav">
          ...
          {/* Only show extra tabs for logged-in users */}
          {!guest && user && (
            <>
