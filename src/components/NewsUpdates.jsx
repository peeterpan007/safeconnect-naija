...
function NewsUpdate({ user }) {
  ...
  return (
    <div style={{ marginBottom: "20px", padding: "15px", background: "#fff", borderRadius: "8px", boxShadow: "0 2px 6px rgba(0,0,0,0.1)" }}>
      ...
      <h3 style={{ marginTop: "20px" }}>News Feed</h3>
      <ul>
        {db.news.map(n => (
          <li key={n.id} style={{ marginBottom: "10px", padding: "8px", border: "1px solid #eee", borderRadius: "5px" }}>
            <strong>{n.title}</strong><br/>
            {n.description}<br/>
            {n.link && <a href={n.link} target="_blank" rel="noreferrer">{n.link}</a>}
          </li>
        ))}
      </ul>
    </div>
